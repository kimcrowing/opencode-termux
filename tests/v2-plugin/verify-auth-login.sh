...
#      present, proving namespace composition over all 10 tools.
#
...
EXPECTED_IDS="auth_login_sites auth_login_login auth_login_status auth_login_token auth_login_refresh auth_login_logout auth_login_run_activities auth_login_manual_token auth_login_render_qr auth_login_qr_image_path"
if [[ ! -f "$WORK/auth-login-sentinel.json" ]]; then
