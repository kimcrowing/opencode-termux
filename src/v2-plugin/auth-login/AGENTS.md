...
- ★★ **网络搜索旁证（2026-09-10，websearch 实证，佐证账号级定性并给出官方解除路径）**：
  ① aq.jd.com 原文即「您的账号存在安全风险，暂无法在京东网页端使用。请使用该账号登录【京东商城 APP】
  完成安全验证」——官方强制风控页，无技术绕过；② 京东《违规账号处理规则》
  （help.jd.com/user/issue/164-3445.html）明确把「**借助软件、机器登录京东账号**」列为违规情形，
  处理措施含「强制风险验证；限制注册、登录、领券、下单」——我们的自动化扫码+每日脚本正是该规则
  触发对象，解释 p 参数稳定一致（规则化打标，非即时请求指纹）；③ 官方帮助文档
  （help.jd.com/user/issue/164-316.html「账户异常/有风险怎么办」）：自助解除 = 电脑端重置密码 +
  提升安全级别（绑定手机/邮箱、开启支付密码、数字证书）；④ 第三方（2026-07 公开资料）：
  「改密码通常能解决，搞不定联系客服」；有投诉案例显示客服口径含「账号未实名认证」也会弹此页。
  **实操建议（2026-09-10 更新）**：解除 = 修改登录密码（APP「我的-账户与安全」）+ 完善实名/绑定
  手机邮箱 + 提安全级别 → 再在 APP 按提示完成风险验证；**只验一次可能再次触发**（09-09 解验后
  09-10 又拦），因机器登录行为是持续风险信号，建议解除后降低调用频率/间隔。
- ★★ **逆向开发者/社区经验交叉验证（2026-09-10 websearch，回应「要逆向开发者经验而非官方文档」）**：
  官方文档之外，逆向/薅羊毛社区对「账号存在安全风险/网页端无法登录」有大量同款实证，结论互相印证：
  - **现象完全同款（V2EX t/1177973，2025-12）**：用户挂 6dy 京东脚本自动做任务 → cookie 失效后
    网页端（m.jd.com 与整个 jd.com）密码/扫码/短信**全部无法登录**，改密码无效；楼下多名用户回复
    「我也是一样，只能 APP 登录，网页、小程序均无法登录」→ 与 loon520 七次扫码全 1100 现象
    **一字不差**，证明 1100 = 社区公认的「网页端全面封锁」账号标记，不是我们的抓包姿势问题。
  - **无公开技术绕过（关键负结论）**：V2EX t/1142616（2025-07）客服口径「京东账号安全系统**人工无法介入**」、
    「等一个月再试」；全网（52pojie/V2EX/CSDN/掘金/GitHub）**无任何公开的 1100/风控页绕过方案**——
    最接近的逆向帖（h5st/waap/x-api-eid-token）只解决接口签名与风控参数，**解决不了账号级登录标记**。
  - **h5st 顶级逆向作者（舟涯 blog.zhx47.top）亲述**：早已放弃京东自动化——「京东现在没什么好撸的了，
    动不动掉线，env不对直接踢下线。部分接口除了踢下线还要拉黑你。即使这个签名生成的与官方百分百一致，
    还是可以很轻松的查到你，毕竟你的cookie、埋点都不对」→ 检测维度 = 账号行为 + 埋点/环境一致性，
    与「chromium 完整浏览器栈仍 1100」互相印证：**单点无损伪装无意义，风控在账号层**。
  - **设备/IP 也会被拉黑（线报酷 2026-05 吧友实测）**：「京东更新机制了不是风控账号而是手机/设备」
    「换流量登录，现在京东黑ip」「设备黑了，换设备」——若 APP 解验后仍 1100，可换网络出口
    （手机流量/其他宽带）与换设备再扫码验证。
  - **解除路径社区共识 = 三种，全部绕不开「人」**：
    ① APP 内完成安全验证（人脸/短信，唯一官方通道；V2EX 网友指出验证协议实际落在**京东金融**体系，
    非商城）；② 打 **95118（京东金融客服）** 要求改用**语音验证码**替代人脸——V2EX t/1142616、
    t/1156669 多名网友实测 1~2 天内成功（950618 商城客服无用，只会「向上反馈」，已多人证伪）；
    优先试这条可免去人脸环节；③ 12315/网信办投诉（次选，耗时长）。**验证解除后网页端登录仍可能
    被拦**（t/1177973 客服反馈需技术处理 24h），需耐心重试。
  - **社区对「长寿命登录态」的共识（52pojie 常家大公子）**：PC 端 thor ck 有效期短（与实测 ~10.5h
    一致）；「APP 端扫码登录的 CK 有效期是永久的」→ 最终可行路径仍是 APP 端拿登录态走
    manual_token 注入，并尽量拉长续期间隔。
  **结论（2026-09-10 定稿，官方文档+逆向社区双源一致）**：1100 = 纯账号级安全标记；**无技术绕过**，
  解除只能走 APP 官方验证（或 95118 语音码替代）；机器登录是持续触发源，解除后必须降频
  （30min 调度太密，建议每日一次或 12h 以上）。
