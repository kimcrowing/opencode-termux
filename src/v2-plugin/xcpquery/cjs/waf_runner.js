...
function computeCookie(cd, nsd, wo) {
  return new Promise((resolve, reject) => {
    const { sandbox, captured, documentObj } = buildSandbox();
    if (wo) documentObj._c = String(wo);
    let vmErr = null;
    try {
      vm.createContext(sandbox);
      // Tolerant String.prototype.toString (waf.js coerces non-strings via it).
      vm.runInContext('String.prototype.toString = function(){ try { return String(this); } catch(e){ return Object.prototype.toString.call(this); } };', sandbox);
      vm.runInContext(`
        var _$brls_wc = 0;
      ...
      `, sandbox);
      vm.runInContext('window.$_ts = window.$_ts || {}; window.$_ts.nsd = ' + Number(nsd) + '; window.$_ts.cd = ' + JSON.stringify(String(cd)) + ';', sandbox);

      const wafCode = wrapCalls(fs.readFileSync(WAF_JS, 'utf8'));
      vm.runInContext(wafCode, sandbox, { filename: 'waf.js' });
    } catch (e) {
      vmErr = e;
    ...
    try {
      if (vm.runInContext('typeof _$oy', sandbox) === 'function') vm.runInContext('_$oy();', sandbox);
    ...
    sandbox.setTimeout(function () {
      ...
      if (!cleanCookie) { reject(new Error('waf.js produced no cookie')); return; }
      resolve(cleanCookie);
    });
  });
}
...
if (require.main === module) {
  ...
  const wo = process.argv[4] || process.env.WAF_WO || '';
  if (!cd) { console.error('usage: node waf_runner.js <cd> <nsd> [wo]'); process.exit(1); }
  computeCookie(cd, nsd, wo).then(c => { console.log('BARECOOKIE|' + c); })
